...
## 7. Melhorias Recentes e Detalhes de Implementação

### 7.1 Gestão de Pedidos
- **Prevenção de Relançamento**:
    - O sistema impede que pedidos já lançados sejam enviados novamente para separação enquanto estiverem em progresso.
    - Relançamento (recontagem) só é permitido para pedidos com status finalizado (`separado`, `conferido`, `finalizado`) ou `cancelado`.
- **Filtros Avançados**: Busca por ID, período, status financeiro, roteirização e prioridade.

### 7.2 Otimização da Separação (Handheld)
- **Scanner Global (Keyboard Handling)**:
    - Implementação de listener global para captura de input de leitores de código de barras.
    - Captura de teclas rápida (< 50ms) para distinguir leitura de digitação manual.
    - Prevenção do surgimento do teclado virtual em inputs não intencionais.
- **Feedback ao Operador**:
    - Feedback visual (Toasts) e sonoro (opcional) imediato após leitura.
    - Scroll automático para o item lido na lista.
- **Agrupamento de Itens**:
    - Itens do mesmo SKU no mesmo pedido são visualizados de forma agregada para otimizar o fluxo de separação (picking).

### 7.3 Visão em Loja
- **Painel de Gestão para Lojas**:
    - Visualização de metas e performance de vendas em tempo real.
    - Redirecionamento automático para perfil de Gestor.
    - Gráficos com valores faturados expostos para fácil leitura em telas grandes (TVs).
